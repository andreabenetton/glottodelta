...
(()=>{
  ...
  /* Phone chart layout toggle (list <-> classic grid). Display-only lens like the
     accessibility classes; deliberately not serialized to the URL. */
  const chartLayoutToggle=document.getElementById('chartLayoutToggle');
  if(chartLayoutToggle)chartLayoutToggle.addEventListener('click',()=>{const grid=document.body.classList.toggle('chart-grid-view');chartLayoutToggle.setAttribute('aria-pressed',String(grid));chartLayoutToggle.textContent=grid?'Switch to the stacked list':'Switch to the classic chart grid';setStatus(grid?'Classic chart grid enabled.':'Stacked list view enabled.');});
  ...
  /* A click on the dialog element itself (not its children) is a backdrop tap. */
  provenanceDialog.addEventListener('click',event=>{if(event.target===provenanceDialog&&typeof provenanceDialog.close==='function')provenanceDialog.close();});
