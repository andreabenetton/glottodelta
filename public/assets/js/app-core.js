...
const drawer=document.getElementById('drawer'), list=document.getElementById('langList'), lsearch=document.getElementById('langSearch'); let current=[], currentSymbol='', drawerOpener=null;
...
/* Single close path for every dismissal (Close button, Escape, backdrop, popstate).
   Wrapped by technical-features.js to also sync the URL — call the identifier, never
   a captured copy. */
function closeDrawer(){drawer.classList.remove('open');drawer.setAttribute('aria-hidden','true');currentSymbol='';if(drawerOpener){drawerOpener.focus({preventScroll:true});drawerOpener=null;}}
