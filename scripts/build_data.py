...
def load_allophones():
    """Documented-allophone map (data/allophones.json, from build_allophones.py).

    symbol -> glottocodes where PHOIBLE documents the symbol as an allophone of
    a DIFFERENT phoneme. Phonetic-level evidence for the "phonetic view" toggle;
    never merged into SYMBOL_LANGS, so L and P are untouched.
    """
    path = os.path.join(ROOT, 'data', 'allophones.json')
    if not os.path.exists(path):
        return {}, {}
    raw = json.load(open(path, encoding='utf-8'))
    meta = raw.get('_meta', {})
    return raw.get('symbols', {}), {
        'documentedLanguages': meta.get('languages_with_allophone_docs', 0),
        'valuetableLanguages': meta.get('languages_in_valuetable', 0),
        'release': meta.get('phoible_release', ''),
    }
...
def main():
    ...
    supp_langs_obj = json.dumps(SUPP_LANGS, ensure_ascii=False, sort_keys=True,
                                separators=(',', ':'))
    # Documented-allophone additions (phonetic view). Map glottocodes to language
    # ids; prefer the ISO-coded entry when several ids share a glottocode; drop
    # languages already phonemically attested for the symbol (they are in L).
    ALLO_SYMBOLS, ALLO_META = load_allophones()
    ids_by_glottocode = {}
    for i, l in enumerate(languages):
        ids_by_glottocode.setdefault(l['g'], []).append(i)
    allophone_langs = {}
    for sym, glottocodes in ALLO_SYMBOLS.items():
        attested = set(symbol_langs.get(sym, []))
        ids = set()
        for g in glottocodes:
            candidates = ids_by_glottocode.get(g)
            if not candidates:
                continue
            iso_coded = [i for i in candidates if languages[i]['iso'] not in PLACEHOLDER_ISO]
            ids.add((iso_coded or candidates)[0])
        extra = sorted(ids - attested)
        if extra:
            allophone_langs[sym] = extra
    allophone_obj = '{' + ','.join(
        json.dumps(s) + ':[' + ','.join(map(str, ids)) + ']'
        for s, ids in sorted(allophone_langs.items())) + '}'
    allophone_meta_obj = json.dumps(ALLO_META, sort_keys=True, separators=(',', ':'))
    ...
    data_js = (
        ...
        "const ALLOPHONE_LANGS=" + allophone_obj + ";\n"
        "const ALLOPHONE_META=" + allophone_meta_obj + ";\n"
...
- Documented-allophone map (`ALLOPHONE_LANGS`, phonetic view): **{sum(len(v) for v in allophone_langs.values()):,}**
  (symbol, language) pairs over {len(allophone_langs)} symbols, from PHOIBLE {ALLO_META.get('release','—')}
  allophone documentation covering {ALLO_META.get('documentedLanguages',0):,} of
  {ALLO_META.get('valuetableLanguages',0):,} ValueTable languages. Display-only; L/P unchanged.
